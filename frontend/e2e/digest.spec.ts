...
test.describe("Digests", () => {
  ...
  test("clicking digest shows summary and highlights", async ({ context, page }) => {
    ...
    // Highlights
    await expect(page.getByText("AI developments continue")).toBeVisible();

    // Topics section should render
    await expect(page.getByText("Top Topics")).toBeVisible();

    // Included emails
    await expect(page.getByText("Included Emails")).toBeVisible();
    await expect(page.getByText("AI News This Week")).toBeVisible();
  });
