...
func (r *WeeklySummaryRepo) ListByUser(ctx context.Context, userID string, limit, offset int) ([]domain.WeeklySummary, int, error) {
	...
	rows, err := r.pool.Query(ctx,
		`SELECT id, user_id, period_start, period_end, email_count, top_senders, categories, action_items, sent_at, created_at
	...
	for rows.Next() {
		...
		if err := rows.Scan(
			&s.ID, &s.UserID, &s.PeriodStart, &s.PeriodEnd, &s.EmailCount,
			&topSenders, &categories, &s.ActionItems, &s.SentAt, &s.CreatedAt,
		); err != nil {
