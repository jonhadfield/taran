...
func (r *WaitlistRepo) GetByID(ctx context.Context, id string) (*domain.WaitlistRequest, error) {
	row := r.pool.QueryRow(ctx,
		`SELECT id, email, created_at
		 FROM waitlist_request WHERE id = $1`, id)
	...
	if err != nil {
		return nil, fmt.Errorf("get waitlist request by id: %w", err)
	}
	return &req, nil
}
