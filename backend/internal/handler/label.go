...
func (h *LabelHandler) Create(w http.ResponseWriter, r *http.Request) {
	...
	// Enforce max label count per user
	existing, _ := h.Labels.ListByUser(r.Context(), userID)
	if len(existing) >= 50 {
		WriteError(w, http.StatusBadRequest, "maximum 50 labels allowed")
		return
	}

	// Validate color against known palette
	if req.Color != "" {
		validColors := map[string]bool{"blue": true, "green": true, "red": true, "purple": true, "orange": true, "yellow": true, "pink": true, "cyan": true}
		if !validColors[req.Color] {
			WriteError(w, http.StatusBadRequest, "invalid label color")
			return
		}
	}
